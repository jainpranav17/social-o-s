...
function CaptionStudio() {
  ...
  const saveApiKey = () => {
    if (apiKeyInput.trim()) {
      ...
      toast.success("Cleared custom Gemini API key.");
    }
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="rounded-xl border border-border bg-card px-3.5 py-2 text-xs font-semibold shadow-sm transition hover:bg-secondary flex items-center gap-2 cursor-pointer"
          >
            <Sparkles className="h-3.5 w-3.5 text-primary" /> AI Key Settings
          </button>
      ...
      {showSettings && (
        <div className="rounded-2xl border border-primary/30 bg-primary/5 p-5 space-y-3">
          <h3 className="font-display text-sm font-semibold flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-primary" /> Configure Gemini API Key
          </h3>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Enter your Google Gemini API key below to enable AI caption generation directly in your browser.
          </p>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 rounded-xl border border-border bg-background px-3.5 py-2 text-xs outline-none focus:border-primary"
            />
            <button
              type="button"
              onClick={saveApiKey}
              className="rounded-xl bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground transition hover:opacity-90 cursor-pointer"
            >
              Save Key
            </button>
          </div>
